...
#if defined(NDEBUG)
#error "Erotica cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zZTA spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zZTA spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zZTA spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
      ...
      if (nHeight == 0) {
          ...
          nSubsidy = 2.0 * COIN;
      } else if (nHeight <= 262800 && nHeight > 600) {
          nSubsidy = 6.9 * COIN;
          ...
          nSubsidy = 0.9 * COIN;
      } else if (nHeight <= 2365200 && nHeight > 2102400) {
          nSubsidy = 0.6 * COIN;
      } else if (nHeight <= 2628000 && nHeight > 2365200) {
          nSubsidy = 0.2 * COIN;
      } else if (nHeight <= 2890800 && nHeight > 2628000) {
          nSubsidy = 0.2 * COIN;
      } else if (nHeight <= 3153600 && nHeight > 2890800) {
          nSubsidy = 0.1 * COIN;
      } else if (nHeight <= 3416400 && nHeight > 3153600) { 
          nSubsidy = 0.1 * COIN;
      } else if (nHeight <= 3679200 && nHeight > 3416400) {
          nSubsidy = 0.1 * COIN;
      } else if (nHeight <= 3942000 && nHeight > 3679200) {
          nSubsidy = 0.1 * COIN;
      } else if (nHeight <= 4204800 && nHeight > 3942000) {
          nSubsidy = 0.1 * COIN;
      } else if (nHeight <= 4467600 && nHeight > 4204800) {
          nSubsidy = 0.1 * COIN;
      } else if (nHeight <= 99000000 && nHeight > 4467600) {
          nSubsidy = 0.1 * COIN;
      } else {
          nSubsidy = 0.1 * COIN;
      }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	// 69% for Masternodes from block 600 
	if (nHeight <= 600) {
	      ret = blockValue  / 100 * 0;               // %0 for Masternode in PoW
	} else if (nHeight > 600 && nHeight <= Params().getFunDevForkBlock() ) {
		  ret = blockValue  / 100 * 76;               // %76 for Masternode (Switchover Point - 2 blocks)
	} else if (nHeight > Params().getFunDevForkBlock() ) {
		  ret = blockValue  / 100 * 76;               // %76 for Masternode +- DevFund =MN69% DEV=7% POS=24%
	}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Erotica
...
{
    RenameThread("erotica-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZEROTICAMinted()
{
...
void RecalculateZEROTICASpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zZTA supply
...
bool RecalculateEROTICASupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zZTA with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zZTASpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert EROTICA to zZTA
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zZTA spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zZTA serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zZTA mints and %d zZTA spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Erotica: We use certain sporks during IBD, so check to see if they are
