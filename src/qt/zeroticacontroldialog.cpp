...
#include "ui_zeroticacontroldialog.h"
...
std::list<std::string> ZEroticaControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZEroticaControlDialog::listMints;

ZEroticaControlDialog::ZEroticaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZEroticaControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZEroticaControlDialog::~ZEroticaControlDialog()
{
    delete ui;
}

void ZEroticaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEroticaControlDialog::updateList()
{
...
void ZEroticaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEroticaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZErotica_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEroticaControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZEroticaControlDialog::GetSelectedMints()
{
...
void ZEroticaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
