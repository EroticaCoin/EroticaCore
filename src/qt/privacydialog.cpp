...
#include "zeroticacontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zZTA ought to be enough for anybody." - Bill Gates, 2017
    ui->zZTApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzZTASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzZTA->setEnabled(false);
        ui->pushButtonMintzZTA->setToolTip(tr("zZTA is currently disabled due to maintenance."));

        ui->pushButtonSpendzZTA->setEnabled(false);
        ui->pushButtonSpendzZTA->setToolTip(tr("zZTA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zZTApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzZTA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZTA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zZTA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zZTA in ") +
...
void PrivacyDialog::on_pushButtonSpendzZTA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZTA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zZTA
        sendzZTA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zZTA
    sendzZTA();
}

void PrivacyDialog::on_pushButtonZEroticaControl_clicked()
{
    ZEroticaControlDialog* zZTAControl = new ZEroticaControlDialog(this);
    zZTAControl->setModel(walletModel);
    zZTAControl->exec();
}

void PrivacyDialog::setZEroticaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzZTASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzZTA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Erotica Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zZTApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zZTA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EROTICA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zZTA</b>";
    ...
    // use mints from zZTA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZEroticaControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZEroticaControlDialog::GetSelectedMints();
    }
    ...
    // Clear zerotica selector in case it was used
    ZEroticaControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zZTA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Erotica, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zZTA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zZTA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zZTA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zZTA "));
    ui->labelzZTAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzZTASyncStatus->setVisible(fShow);
}
